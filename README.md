## Organizer for drill and hex bit

# Описание проекта

Данный проект представляет собой модульный органайзер для сверл и бит, собранный из комбинации готовых конструкционных деталей и деталей, напечатанных на FDM 3D-принтере. Органайзер позволяет удобно хранить сверла разных диаметров, а также легко изменять конфигурацию по мере необходимости.

Органайзер состоит из вертикальных и горизонтальных профилей, соединенных между собой напечатанными элементами, и держателей (фишек) для каждого размера сверл. Все элементы можно собирать, разбирать и переставлять, что делает органайзер гибким и удобным для пользователя.

# 🧱 Материалы и комплектующие

Для сборки органайзера использовались следующие элементы:

- Алюминиевый конструкционный профиль: V20×20L
- Сухарь пазовый: М4, паз 6
- Винты: М4
- Детали, напечатанные на FDM 3D-принтере (соединительные элементы и держатели для сверл)
- Профили нарезаны на нужную длину для сборки органайзера

# 🛠️ Конструкция и сборка

Органайзер состоит из нескольких ключевых частей:

### 1) Вертикальные профили
Два основных профиля, расположенные вертикально, служат каркасом конструкции.  

Снизу к ним крепятся **опорные лапы**, которые стягиваются винтами М4. Зафиксированы они так, что не проворачиваются благодаря выступам, входящим в пазы профилей.

<img width="1048" height="610" alt="image" src="https://github.com/user-attachments/assets/7391f96a-b39a-4fb9-b542-90a9c339ed31" />

<img width="588" height="664" alt="image" src="https://github.com/user-attachments/assets/de5be331-1235-4fde-b694-87b23ff61024" />

<img width="524" height="509" alt="image" src="https://github.com/user-attachments/assets/9c1f7ca0-83eb-46ad-a720-0ab3a2a9b8ee" />

### 2) Горизонтальные профили:
Горизонтальные профили расположены на **трёх уровнях** и соединены с вертикальными профилями через напечатанные крепления и сухари.

<img width="1006" height="622" alt="image" src="https://github.com/user-attachments/assets/dfbe58ff-05bb-416f-bd5b-cd6d7904cb3f" />
Аналогично вертикальным, они фиксируются на торцах, чтобы держать устойчивую конструкцию.
<img width="341" height="454" alt="image" src="https://github.com/user-attachments/assets/fe143ae4-5376-4052-8d96-6f915fa6cd48" />
<img width="397" height="611" alt="image" src="https://github.com/user-attachments/assets/0509d7b7-9be4-4e6d-9c96-df8ef44d949c" />

Горизонтальные профили удерживают фишки с отверстиями под сверла.
<img width="948" height="581" alt="image" src="https://github.com/user-attachments/assets/02803999-108e-46b2-a1c6-f6e969218e53" />

### 3) Фишки для сверл:

- Фишки вставляются в пазы горизонтальных профилей.
 Фишки могут свободно передвигаться по профилю, но не выпадают благодаря форме выступов.
<img width="1012" height="522" alt="image" src="https://github.com/user-attachments/assets/c8e69b6c-e5cd-4ef9-8792-96a5f5328481" />

- Каждая фишка рассчитана под конкретный диаметр сверла или шестигранника.
При необходимости фишки можно **добавлять, удалять или переставлять**.
<img width="873" height="620" alt="image" src="https://github.com/user-attachments/assets/65a1f62a-5fdc-41dc-acc2-6718ea138a8a" />

Таким образом органайзер является **модульным и адаптивным**, что позволяет подстраивать его под любые наборы инструментов.

# 🔄 Модульность и настройка

- Возможность добавления или удаления фишек под конкретные сверла
  <img width="1280" height="960" alt="image" src="https://github.com/user-attachments/assets/7c207fba-fbb4-48ab-b1b3-908d6d40af88" />

- Конструкция легко масштабируется для других размеров инструментов
- Все напечатанные элементы можно легко изменить в CAD для адаптации под новые потребности

# 📸 Фото и визуализация
<img width="1280" height="960" alt="image" src="https://github.com/user-attachments/assets/2832daf7-70c2-4c2d-a3da-69c7a42b0f61" />

<img width="1280" height="960" alt="image" src="https://github.com/user-attachments/assets/60997991-21e4-49ad-a810-98743465f462" />

<img width="1280" height="960" alt="image" src="https://github.com/user-attachments/assets/8ba99088-c7ac-46fe-81e4-f148796370c7" />

<img width="960" height="1280" alt="image" src="https://github.com/user-attachments/assets/6bc04cd3-7f5b-421b-84fd-c30c46eadaf2" />

<img width="960" height="1280" alt="image" src="https://github.com/user-attachments/assets/ad6c016b-1f5e-4234-b055-01769347b26e" />

![Uploading image.png…]()

## 📁 Файлы проекта
В репозитории присутствуют:

- STEP и SOLID фалй для печати соединительных элементов и фишек  
- Изображения сборки и деталей  
- Опционально: CAD-файлы для редактирования


